/*
  ...
  SSSSSSSSSSS     AAAAAAAAAAA    TTTTTTTTTTTTT    AAAAAAAAAAA    NNN       NN
 SSSSSSSSSSSSS   AAAAAAAAAAAAA   TTTTTTTTTTTTT   AAAAAAAAAAAAA   NNNNN     NN
 SS         SS   AA         AA        TTT        AA         AA   NN  NN    NN
 SS              AA         AA        TTT        AA         AA   NN   NN   NN
  SSSSSSSSSSS    AAAAAAAAAAAAA        TTT        AAAAAAAAAAAAA   NN    NN  NN
    SSSSSSSSSS   AA         AA        TTT        AA         AA   NN     NN NN
 SS         SS   AA         AA        TTT        AA         AA   NN      NNNN
 SSSSSSSSSSSSS   AA         AA        TTT        AA         AA   NN       NNN
  GSSSSSSSSSS    AA         AA        TTT        AA         AA   NN        NN

   Nome      : MrSatan.r  (29-10-2000)

   Autore    : Simone Ascheri

   Scopo     : E' convinto di essere l'uomo piu' forte del pianeta, e quindi
               si e' offerto di sconfiggere Cell.

LA STORIA
=========

Sconfitto Freezer Goku inizia il viaggio di ritorno sulla Terra. Purtroppo
impiega piu' tempo del previsto e il nemico, sotto forma di Metal Freezer,
lo precede sul pianeta, insieme al padre Cold, per vendicarsi.
Tutto e' perduto?
Nemmeno per sogno, poiche' arriva un misterioso guerriero che, trasormatosi
in SuperSayian, distrugge con facilita' i due.
All'arrivo di Goku si rivela come Trunks, figlio di Vegeta, tornato indietro
nel tempo per avvisare la squadra Z di una nuova minaccia (i cyborg del dottor
Gelo) e dare una medicina a Ka_aroth.
Questi infatti morira' di un attacco di cuore nel futuro, prima di poter
affrontare i nemici. Questi non troveranno resistenza alcuna e stermineranno
la razza umana.
La squadra Z inizia subito gli allenamenti, ma il viaggio a ritroso di Trunks
ha gia' cambiato il corso della storia, che non si sviluppa piu' come il
Sayian aveva predetto.
Infatti dal futuro arriva un nuovo Cyborg del dottor Gelo, completamente sconosciuto
per il giovane Trunks.
Ha viaggiato nel tempo con lo scopo di trovare C17 e C18, i primi due androidi, che
nella sua epoca Vegeta aveva distrutto (altro cambio delle ramificazioni storiche).
Dopo averli assorbiti si trasforma nell'essere perfetto, pronto a distruggere
l'universo.
Ne' Trunks, ne' Crilin, ne' Junior ne' Vegeta sembrano in grado di tenergli testa,
ma prima che Goku possa iniziare a battersi sale sul ring un nuovo, fortissimo
(almeno cosi' lui si crede) atleta..........
MrSatan:-)
 
COMPORTAMENTO
=============

Data la mancanza di tempo, il robot e' esattamente uguale a Vegeth.r, con la
sola eccezione della routine d'attacco finale e dello spostamento di angolo in
angolo, che non sempre avviene con moto epilettico.

Fase Iniziale
-------------
All' inizio del Match MrSatan cerca un angolo libero, cominciando da quello
...
per l'occasione ribattezzato MalDiPancia.
...
A) Se i danni sono inferiori all'80%:
   ...
   ===================================
     ...
     -MrSatan li cerca innanzitutto nell' angolo seguente, e in caso affermativo
      li attacca...
...
==================================
   ...
   - Oscilla sulla bisettrice del quadrante con brevi movimenti: in pratica
     ...
     anno, con la differenza che MrSatan utilizza, in questa situazione, la
     routine di fuoco piu' precisa di cui dispone: la sfera GenkiDama().
  
In ogni caso, se MrSatan subisce troppi danni o ha un nemico troppo vicino
cambia posizione.

Fase Finale
-----------
Ogni 10000 cicli circa MrSatan conta gli avversari:

 1 * Se c' e' piu' di un superstite continua il movimento cambiando angolo
     se ha subito troppi danni.
 2 * Se invece si accorge di avere solo piu' un avversario parte con l'attacco
     finale, il CalcioInFaccia.
     Si tratta della mia visione del movimento di Satana.r, che sembra dare
     risultati migliori del SuperVegeth/MalDiPancia in torneo9198.
...
Il robot utilizza quattro diverse routine di fuoco:
  ...
  - Il SuperSayan() e' quasi completamente nuovo:
    ...
    3 - la correzione avviene ora sul primo angolo di scansione, non piu' sull'ultimo.
  - La GenkiDama() e' la routine piu' precisa di cui dispone MrSatan.
...
Lo spostamento da un angolo all'altro avviene, a differenza che in Vegeth,
...
sopra: il robot (attacco finale a parte) era pronto 3 mesi fa, e da allora
l'ho riguardato pochissimo.
...
int att, max, clock, gira, time, klik, scn;
     ...
     if (!Ricerca(ang=ang2))                 /*La scelta e' effettuata in maniera
       ...
       if (!Ricerca(ang=ang2+630))
         Arrivo(OooHoo(ang2+=90),dy=park);
              ...
              {
                if (!Ricerca(ang=ang2+315))
                  {
                     ang2+=180;
                     Arrivo(dx=1000-dx,dy=1000-dy);
                  }
                else
                  {
                     while (++dan<=damage()) Senzu();
...
int z;
  {
    NuvolaSpeedy(time=500,500);
    vang=((ango+180)/90)*90;
    while (drive(ang=vang,att=100))
           {
            SuperSayian();
            if (--gira)
              {
                if (((loc_x()%700)<300)||(z=((loc_y()%700)<300)))
                  vang=180*(!z)*(loc_x(gira=3)>500)+(90+180*(loc_y()>500))*z;
              }
            else
              {
                vang=(((ango+180)/90)+(clock^=1))*90;
                gira=3;
              }
            if ((gira==3)&&(ang!=vang)) KameAmeA();
          }
  }
