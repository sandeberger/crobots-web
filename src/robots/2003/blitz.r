...
E' una minima evoluzione di mind.r, di cui eredita tutti i difetti.
...
spara()
{
        ...
        else if (x=scan(ang-=60,10));
        else if (x=scan(ang+=80,10));
        else if (x=scan(dir,10)) ang=dir;
        else {return ang+=40;}
        cannon (ang+=7*(!(scan(ang+356,7)))+353*(!(scan(ang+4,7))), 2*scan(ang,10)-x );      
        if(x>mx) return(ang+=41);
} 
