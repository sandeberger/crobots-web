...
rompo una tradizione, usando un nome "umano" al mio robot.
Ma un campione come lui, che vinceva il titolo mondiale mentre ci lavoravo, meritava la dedica.


 Michelangelo Messina


Robot senza nessuna pretesa, se non di tentare di rimanere in vetta al KingOfTheHill come erede di Suddenly.r
...
mai.
Se e' colpito cerca un eventuale lato libero dove spostarsi.
...
*/
    ...
    while (dam=damage()+10) {
        while ( (damage() < dam) ) { /*fino quando non e' colpito o limite di tempo*/
                if (dx) xmin(xyf(dir0=deg=dir=180));
                else xmag(xyf(dir0=deg=dir=360));
                if(orng>740) {if (ne&&t<1200); else attacco();}
                if (up) ymin(xyf(dir0=deg=dir=270));
                else ymag(xyf(dir0=deg=dir=90));
		attacco();
...
xyf()  { sk=1;spara(drive(dir,100)); if(t>1200&&dam<69) fuoco(drive(dir,100));spara(drive(dir,100));stop(sk=0);dir+=180;} 
...
boom()
{
            while(1) {
                if(t%2) {
			if(orng>650) dir=deg+25+(b^=1)*235;
			else if(orng>210) dir=deg+80+(b^=1)*190;
                        else dir=(deg/90)*90;                               
