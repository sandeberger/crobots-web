...
La modifica principale è il cambio di routine di fuoco.
E' risultato quasi un gemello omozigota degli scarsissimi unlimited.r e back.r
...
Purtroppo i micro proprio non mi piacciono, e anche quest'anno tenterò di fare peggio dell'anno prima...
...
{
        if(dx) {
                if(occupato(170)); else dove(xmag(85));
        } else {
                if(occupato(350)); else dove(xmin(915));
        }
}
...
attacco()
{
                ...
                if (x>16) {
                        sc1=5; sc2=3;  ++ff;  
                        while(1) {ymag(350);xmin(650); ymin(650);xmag(350); }
                }
}
