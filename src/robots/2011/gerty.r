...
That's an improved version of Jedi5. Differences: shrinked f2f boom-style of
Copia and an optimized movement routine.

*/


int ang,oang,range,orange,drange,dir,posx,posy,deg,enemy,dist,l,zdeg,timer,n,m;

int _cannon_() /* save memory space routine */
{
  return cannon(ang+(ang-oang),2*scan(ang,10)-range);
}

firef2f(dir) /* wall-e fire */
{
        drive(dir,100);
        if (range=scan(oang=ang,10)) {
                if (scan(ang-8,5))  { if (scan(ang-=5,2)) ; else ang-=4; return _cannon_(); }
                if (scan(ang+8,5))  { if (scan(ang+=5,2)) ; else ang+=4; return _cannon_(); }
                if (scan(ang,10))   { if (scan(ang-=2,2)) ; else ang+=4; return _cannon_(); }
        }
