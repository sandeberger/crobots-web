/**************************************************************************/
/***   GOLEM2.R   ( Crobot per il Settimo Torneo di MCmicrocomputer )   ***/
/***                                                                    ***/
/***   Autore    :  Stefano Francesi                                    ***/
/***                                                                    ***/
/***   Creazione :  30.09.1996                                          ***/
...
/** trovato, tenta di tenerlo continuamente sotto tiro con una serie     **/
/** di colpi mirati.                                                     **/
/** Quando ritiene di essere rimasto solo con l'ultimo nemico si         **/
/** avventa su di lui prima che possa scappare.                          **/
/**************************************************************************/
...
nella_mischia()
{
   ...
   /*** Divento un temibile (!) inseguitore ***/
   while (!scan(scan2, 10)) scan2+=21;
   drive(scan2,40);
   while(1)
   {
      if (rangefuoco=scan(scan2,10)) cannon(scan2,rangefuoco);
      else
      {
         drive(scan2,40);
         if (scan(scan2+180,10)) scan2+=180;
         else 
         {
            scan2+=318;
            while(!(rangefuoco=scan(scan2+=21,10)));
            cannon(scan2,rangefuoco);
         }
         while (speed()>49) ;
         drive(scan2,95);
