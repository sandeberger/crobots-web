...
SCHEDA TECNICA:
  ...
  Questo robot oscilla nell`angolo N-E, su una linea ortogonale alla
  diagonale S-O/N-E. La sua strategia difensiva si basa sul rapido
...
main()
{
    a=190; r=60; o=60; t=333;                       /* ~420000 cicli */

    while(loc_x()<910) {drive(0,  100); cerca();}   /* --> */
    while(speed() >49) {drive(180,  0); cerca();}
    while(loc_x()>930) {drive(180, 49); cerca();}
    while(speed() > 0) {drive(0,    0); cerca();}

    while(loc_y()<815) {drive(90, 100); cerca();}   /*    */
    while(speed() >49) {drive(270,  0); cerca();}
    while(loc_y()>835) {drive(270, 49); cerca();} 
    while(speed() > 0) {drive(90,   0); cerca();}

  while(--t)                                        /* oscillazione */
   {
    drive(135,100); while(loc_x() >815) cerca();    /* \ */
    drive(315,  0); while(speed() > 49) cerca();   
    drive(315,100); while(loc_y() >815) cerca();    /* \ */
    drive(135,  0); while(speed() > 49) cerca();   
   }
    dir=225; f=0;                                   /* attacco */
  while(1)
   {
