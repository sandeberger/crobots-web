...
Strategia:
	...
	Sceglie l'angolo successivo dove spostarsi se non e' occupato da un altro robot.
	...
	Dopo un numero prefissato di cicli HAL9000 attacca un crobot
...
main()
{
	...
	while(1)
	{
		drive(dir=(90*(!scan(90,10))),100);
		while ((loc_y() < 800) && (loc_x() < 800)) fire();
		stop();
		while (((curx > 500) && (cury < 500)) || ((cury > 500) && (curx < 500)))
		{
			if ((curx > 500) && (cury < 500)) /* (1000,0) */
			{
				drive(dir=135 + (45*(!scan(180,10))),100);
				while((loc_x() > 200) && (loc_y() < 800)) fire();
			} else
			{
				drive(dir=315 - (45*(!scan(270,10))),100); /* (0,1000) */
				while((loc_y() > 200) && (loc_x() < 800)) fire();
			}
			stop();
		}
	}
...
fire()    /* fire() - routine di sparo in movimento */        
{
        if(range=scan(ang,5))
	{
                cannon(ang,range);
