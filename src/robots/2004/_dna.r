...
SCHEDA TECNICA:
  ...
  Il robot e' identico a !Alien (vedere relativa scheda) salvo la parte non toxica 
  dell routine di fuoco.
...
FireXX()
{
  ...
  if (rng=scan(odeg=deg,10)) {
    if (rng>500) return cannon(deg,rng);
...
LookZ()
{ 
  if (timer>180) { if (!Look(zd)) timer=10000; if (damage()<30) timer=10000; }  else if (damage()>40) timer=10000;  
}
