...
M_selim ver. 1.0 (C) Lorenzo Ancarani 2004

Architettura: Midibot
Strategia   : Robot derivato da B_Selim, raggiunge l'angolo piu' vicino e poi oscilla
...
dir=(360+((fx=dx-loc_x(b=(p2)*180+(p1!=p2)*90))<0)*180+atan(((dy-loc_y())*100000)/(fx+(fx==loc_x()))));
   ...
   while (1) { 
	...
	drive(dir+=180,0);oa=b-40;
	  if (r=scan(odeg=a,10))
	  {    
	    if (scan(a+350,10)) a+=350; else a+=10;
	    if (scan(a+350,10)) a+=355; else a+=5; 
	    if (scan(a+355,10)) a+=3; else a+=357; 
	    cannon((a<<1)-odeg+0+0+0+0,(scan(a,10)<<1)-r); 
	  } else {
	        if (r=scan(a+=340,10)) return cannon(a,r);
	        if (r=scan(a+=40,10))  return cannon(a,r);
	        if (r=scan(a+=300,10)) return cannon(a,r);
	        if (r=scan(a+=80,10))  return cannon(a,r);
	        if (r=scan(a+=260,10)) return cannon(a,r);
	        if (r=scan(a+=120,10)) return cannon(a,r);
	        if (r=scan(a+=220,10)) return cannon(a,r);
		if (r=scan(a+=160,10)) return cannon(a,r);
        	if (r=scan(a+=180,10)) return cannon(a,r);
	        a+=270; 	  
		}
    ...
    if((r=scan(a+=339,10)));
      ...
      if((r=scan(a+=42,10)));
        ...
        if((r=scan(d1,10))) a=d1;
          ...
          return (a+=40);
  cannon (a+0+0+0+0,2*scan(a,10)-r);
}
...
Missile()
{
  ...
  {    
    ...
    cannon((deg<<1)-odeg+0+0+0,(scan(deg,10)<<1)-rng); 
  } else {
