...
Torneo di corbots 2004!

Anche le imposte sulle società mutano... è tempo di svecchiare.
...
while (corr=1)
{
	...
	while ((Liquidazione(dx,dy)<65000))
		Sovrattassa();
...
Fuoco(dove)
{
    ...
    if (or=scan(a,10)) 
    {
           ...
           if (scan(a,2)){if ((cannon(a+0+0+0+0+0,3*scan(a,10)-2*or))) return;}
           else if (scan(a-=7,5)){if ((cannon(a-6+0+0,2*scan(a,10)-or))) return;}
           else if (scan(a+=14,5)){if((cannon(a+7,2*scan(a,10)-or))) return;}
           else if (scan(a+=10,5)){if((cannon(a+7,2*scan(a,10)-or))) return;}
    } 
        else if (r=scan(a+=340,10)) return (cannon(a,r));
        else if (r=scan(a+=40,10))  return (cannon(a,r));
        else if (r=scan(a+=300,10)) return (cannon(a,r));
        else if (r=scan(a+=80,10))  return (cannon(a,r));
        else if (r=scan(a+=260,10)) return (cannon(a,r));
        else if (r=scan(a+=120,10)) return (cannon(a,r));
        else if (r=scan(a+=220,10)) return (cannon(a,r));
        else if (r=scan(a+=160,10)) return (cannon(a,r));
        else if (r=scan(a+=180,10)) return (cannon(a,r));
	else a+=270;
	return Fuoco(dove);
}
