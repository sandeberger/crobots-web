...
Multics vorrebbe essere un'evoluzione di Frankie, a cui vorrebbe almeno donare un attacco finale diverso.

Temo che, come l'originale progetto dei BELL LABS (se non erro) sarà un bel buco nell'acqua.

Ok, l'attacco è praticamente lo stesso.
L'unica differenza è che rende di piu' perche' conta gli avversari addirittura prima di muoversi... rischiando grosso di essere colpito.

Strategia
=========

1)Multics all'inizio del match si sposta nell'angolo piu' vicino è controlla se è rimasto solo.
...
3)Quando si accorge che uno ali angoli è disabitato, capisce che lo scontro è per lo meno a tre robot (un vero genio).
...
2)Il conto ali avversari viene fatto in maniera inusuale: anzichè adoperare un ciclo vengono effettuate materialmente tutte le scansioni e tutte le somme, per guadagnare velocità di esecuzione e partire all'attacco prima dei nemici. Sarebbe un'ottima cosa, disponendo di un attacco finale ano di questo nome.
*/
...
Fire()
{
  ...
  {    
           ...
           if (scan(a-=7,4)){if (!(scan(a+=2,2))) a-=4; return(cannon(a+(a-oa),2*scan(a,10)-oldr));}
           if (scan(a+=14,4)){if (!(scan(a-=2,2))) a+=4; return(cannon(a+(a-oa),2*scan(a,10)-oldr));}
           if (scan(a-=7,4)){if (!(scan(a+=2,2))) a-=4; return(cannon(a+(a-oa),2*scan(a,10)-oldr));}
...
conta()
	{
		if(((!scan(ang_pref-34,10))+
			...
			(!scan(ang_pref+71,10))+
			(!scan(ang_pref+92,10))+
			(!scan(ang_pref+113,10))+
			(!scan(ang_pref+134,10)))>7)
				att();
...
Fuoco(dove)
{
    ...
    if (oldr=scan(a,10)) 
    {
           ...
           else if (scan(a+=10,5)){if((cannon(a+9,scan(a,10)))) return;}
	   else a+=20;
    } 
        ...
        else if (rng=scan(a+=200,10)) return (cannon(a,rng));
        else if (rng=scan(a+=140,10)) return (cannon(a,rng));
        else if (rng=scan(a+=240,10)) return (cannon(a,rng));
        else if (rng=scan(a+=100,10)) return (cannon(a,rng));
        else a+=310;
	return Fuoco(dove);
}
