/*
 (praticamente ho preso il commento di pippo3 e ho cambiato una
  decina di parola,
  la modifica piu' sostanziale e' che ho messo due email e
  il nome del sito)

  Pippo4.r

  E' un involuzione di pippo3, che a sua volta era un involuzione di
  pippo2a o pippo2b, non ricordo e sul
  momento non ho tempo per vedere :-)


  Se in presenza di 4 amici all'arena:
     All'inizio si reca all'angolo vicino.
     Si mette ad oscillare a destra e sinistra per il bordo completo
...
main()
{
 ...
 un1=(loc_x(tt=1+40*(od2>1))>500)*300;
 if(od2>1)
 {
  if(loc_y()>500) up(900);
  else dw(100);
/*  vr(100+800*(loc_y()>500));*/
  while(--tt)
  {
   sx(200);
   dx(800);
