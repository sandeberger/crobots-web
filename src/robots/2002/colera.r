...
QUESTO HA 890 ISTRUZIONI MA GAREGGIA NELLA CLASSE 2000

LUCA STRINGHER

COME SI COMPORTA:

Colera va all'angolo più vicino e vibra in diagonale, ogni dieci vibrazioni controlla quanta gente c'è. Quando rimangono in due si comporta in modo differente in base ai danni subiti.
se ne ha pochi corre dietro al'avversario e gli spara, se è oltre il 30 % di danni si sposta al centro e viaggia all'interno di un quadrato tramite la funzione rand() (molto carina) 
...
incubazione(X,Y)
	...
	while(speed()>49)
		{epidemia(k);}
	while(loc_x()<x){drive(b,100);epidemia(k);}
	drive(b,0);
	y=rand(r)+Y;b=90;
	while(speed()>49);
		{epidemia(k);}
	while(loc_y()<y){drive(b,100);epidemia(k);}
	drive(b,0);
	x=rand(r)+X;b=180;
	while(speed()>49);
		{epidemia(k);}
...
febbre()
	...
	if(damage()<35)
		...
		while(1)
			{
			if(scan(ang,10)) {drive(ang,100);cannon(ang,scan(ang,10));}
			else ang+=20;
			}
		}
	else
...
/* CHE VINCA IL MIGLIORE, XKE' L'IMPORTANTE NON E' PARTECIPARE MA VINCERE */ 
