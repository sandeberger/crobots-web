/*
Nome del robot  : Serse
Nome del file   : Serse.r
Autore          : Gianni Ino
...
Altrimenti utilizza la strategia di gioco di Attila.r, leggermente
modificata: infatti sono sparite le routine di fuoco precise di Michelangelo Messina, sostituite
dalla grezzissima fire().
D'altra parte doveva stare in 1000 istruzioni.........
Anche Serse, inoltre, non non si separa mai dall'angolo iniziale.
...
main() {
  ...
  drive(ang=a+180,100);
  while(GeoMetra(dx,dy)>5200) fire();
  Consiglieri();

  Babilonia(ang=(verso=225-180*dw+90*(sx^dw)));
...
Babilonia() {
  fire(fire(fire(fire(fire(fire(drive(verso,100)))))));
  Consiglieri();
  Goniometro(dx,dy);
  while (GeoMetra(dx,dy)>7200) fire(drive(ang,100));
  Consiglieri();
  return ang=verso;
}
...
branchprediction() {
  ...
  while(GeoMetra(dx,dy)<how)
    if (r>800) fire(); else fire();
  Consiglieri(how+=1000);
...
Consiglieri() { fire(fire(drive(ang+=180,0)));}

fire() {
  if (or=scan(a,10)) {
         if (r=scan(a,1))   return cannon(a,2*scan(a,10)-r);
    else if (r=scan(a-5,4)) return cannon(a-=7,2*scan(a,10)-r);
    else if (r=scan(a+5,4)) return cannon(a+=7,2*scan(a,10)-r);
  }
