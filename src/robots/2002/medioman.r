...
/* DOPO AVER RAGGIUNTO IL SUO ANGOLO PREFERITO, OVVERO IL MENO LONTANO,
   ...
   INCREMENTANDONE LE DIMENSIONI DURANTE IL TRASCORRERE DEL MATCH
   ATTACCA SOLO SE CONTRO UN SOLO AVVERSARIO CON UNA ROUTINE MERAVIGLIOSA
   A DIFESA DI TUTTE LE ZITELLE, ANZI LE NUBILI DI QUESTA TERRA
                                                                   */


/*   DONNA, NON TEMERE...
     E' IN ARRIVO MEDIOMAN   */
...
while(1)
{
while(loc_y()>250) {xcc(210,330,300);
                    }
dx(700);
while(loc_y()<750) {xcc(150,30,700);
                     }
sx(300);
}
}
...
xcc(aw,bw,pot){
          while(loc_x()>pot) {Fire2(1,aw);}
          while(loc_x()<pot) {Fire2(1,bw);}
                    }
