...
Come quasi tutti i crobot piu' recenti e furbi Rudolf_7 comincia l'incontro
...
Questa distanza aumenta nel finale se Rudolf_7 non ha troppi danni, in modo
...
Rudolf_7 e' un robot abbastanza "estremo", nel senso che si basa su un
         ...
         if ((t>3&&!(oldr<800)||t>6))
              {
               ...
               if (tempo>5) ss=1;
               if (damage()>87) {ss=0;or=1;oscar=0;}
               flag=ll;
               flag1=2;
               while (flag<ul&&flag1) flag1-=(scan(flag+=20,10)>0);
               t=10002*((flag1));
