...
Descrizione robot:
All'inizio dell'incontro Reuben si posiziona nell'angolo piu' vicino e controlla se è un f2f in questo caso parte subito all'attacco.
...
Reuben utilizza 2 routine di fuoco una veloce per il 4vs4 e 1 mirata allo scontro f2f.
Per farlo rientrare nelle 1000 istruzioni ho dovuto rinunciare alla routine di fuoco toxica di Gantu.
...
int fp,b,x,y,dir,rng,orng,deg,odeg;
...
main()
{
	...
	drive((xd=180*xs)+180,100);
	while(check(xp,3-xs)) ;
	fire(xd+180);  
     	while(speed()>59);
	drive((yd=90+180*ys)+180,100);
	while(check(yp,7-ys)) ;
	fire(yd+180);  
     	while(speed()>59);
