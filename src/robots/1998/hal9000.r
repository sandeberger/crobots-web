/*
        ...
        HAL9000 ver.5.01 del 22/09/98

	Autore: Maurizio Camangi

Strategia:
        Si sposta poi nell'angolo a sud-est del campo di battaglia.
	Oscilla da ovest ad est fino a raggiungere il lato a nord del
        campo di battaglia per poi riprendere il percorso inverso.

	La funzione di fuoco in movimento e' piuttosto complessa
	e si basa sulla correzione dell'angolo e della gittata in base
	allo spostamento proprio e dell'avversario, ispirata dal crobot
	TOX. 
...
main()
{
	...
	while(1)
	{
		while (loc_y() < 800)
		{
			...
			stop();
			drive(dir=180-deg,100);
			while(loc_x() > 200) fire();
			stop();
		}
		while (loc_y() > 200)
		{
			drive(dir=360 - deg,100);
			while(loc_x() < 800) fire();
			stop();
			drive(dir=180+deg,100);
			while(loc_x() > 200) fire();
