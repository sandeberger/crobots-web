...
SCHEDA TECNICA:
Inizialmente il crobot si porta vicino alla cima dello schermo, e da questa
posizione controlla l' angolo NORD-EST; nel caso in cui sia libero lo
raggiunge, altrimenti va ad occupare l' angolo NORD-OVEST. Raggiunta una di
...
inferiore. Durante questa fase (che dura almeno 8E+4 cicli virtuali di CPU)
Vision utilizza 2 diverse procedure di fuoco: kill() che consente
...
Vision puo' attaccare anche se sono presenti 2 (molto raramente 3)
nemici. Non so se in effetti questo sia un inconveniente o no: infatti quando
Vision attacca e' nella zona centrale e puo' essere colpito da
...
danni.
4) Quest' anno ho spedito due crobot (gemelli), ma quello che preferisco
veder combattere nel caso fosse necessario limitare la partecipazione ad un
solo concorrente e' The Invisible Man.
...
crobot di avvicinarsi con piu' precisione ai muri senza spalmarsi;
inoltre negli scontri ravvicinati mortal() non e' molto inferiore a kill().
Il cambiamento di direzione nell' oscillazione e la scelta iniziale dell'
angolo sono ovviamente accorgimenti volti a mantenere Vision fuori tiro
per gli altri crobots.
...
attack()
{
while(loc_y()>550) {drive(270,100);destroy();}
drive(270,0);
...
/* ...LOOK WHAT I 'VE DONE TO MY DREAM... */
