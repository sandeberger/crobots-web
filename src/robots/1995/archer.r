...
/* 
   ...
   Il Robot si puo' comportare in due diversi modi a seconda 
   ...
   spostato "scottava"...). 
   Se i danni subiti superano il 72%, Archer comincia ad
   oscillare lungo il lato su cui si trova cercando di rende-
   re difficile la sua individuazione.
   Per colpire gli avversari Archer utilizza due routine (una 
   quando e' fermo e una quando si muove lungo un lato) cercando
...
int r1,dir;
int ang,d,corner,newdam=0,range,lato,lat90,olddam,dang,ang1,rangef,angf;

main()
{
  /*  Archer si dirige nell'angolo in basso a destra */
  ang=170;
  drive(270,100);
  while (loc_y()>=60);
  ...
  while (d<72)
  {
	  ...
	  if ((newdam=aim_at())!=d)       /* Spara da fermo */
	  {
	    olddam=d;
	    race();
	    if (d>olddam+8)              /* Se viene colpito si muove */
	    {
	      test();
	      if ((d-newdam)>4)
		race();
  ...
  while (1)                              /* Si muove in continuazione */
  {
    race();
  }
}
...
/*
 * Routine di definizione del lato su cui muoversi
 */
