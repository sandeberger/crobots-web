...
HAL9000 v.11 is an improved (!) version of HAL9000 v.7 (2010) which is the
best HAL9000 series robot on the KOTH 4vs4.
Differences:
- Modern book-like F2F
- Simplified routine to count enemies
...
int ang, oang, range, orange, d, dir, posx, posy, x, y, dist, flag, attack, timer, clock, deg1;
...
degree(xx,yy) /* Direzione per x,y */
int xx,yy;
{
    return(dir=(360+((xx-=loc_x())<0)*180+atan(((yy-loc_y())*100000)/xx)));
}
...
corner() /* Si avvicina al corner */ 
{
    ...
    if ( ++timer%12 == 1 )
    {
        ...
        if( damage() < 60 ) attack|=2; else attack=0;
    }
}
...
{
  d=damage()+a;
  while(dista(x,y) < (123450-4925*(damage()>d))) fire(dir);
}
...
main() /* Inizializza alcune variabili ed innesca la routine principale */
{
    ...
    while (1)
    {
        if (radar3(deg1+70*flag))
        {
            dir=deg1 + (30<<flag);

            if (attack & (1<<flag))
            {
                if (damage(lightsaber(5))>d) attack&=~(1<<flag);/* else attack|=(1<<flag);*/
            }
            else
            {
                int t;
                d=damage();
                while((speed()<100) || ((++t<2) && (damage()==d))) fire(dir);
                if(damage()>d)
                {
                    lightsaber(0);
                }
