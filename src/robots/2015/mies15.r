...
/*--------------------------- Torneo: CRobots (2015) ------------------------
  ...
  CROBOT:    mies15.r         (macro)
  CATEGORIA: 2000 istruzioni     (macro)
  AUTORE:    Andrea Jasci Cimini
  
  Code utilization: 86%   (1724 / 2000)
  mies15.r compiled without errors
  ...
  At the beginning of the robot checks whether it is a f2f, if the
  opponents are more than one, rotates clockwise at a distance from the edges
  about one-third the size of the battlefield, after 5 rotations
  It is launched the final attack.
  
 ---------------------------------------------------------------------------*/
  
int a,oa,r,o,d,x,y,b,ne,t,ang;
main()
{
   while(ang<360)
   {
      if(scan(ang+=20,10))
      {
         if(++ne>1)
         {
            ...
            while(1)
            {
               if(r>40) d=a+80+(b^=1)*200; else d+=180;
               x=loc_x(y=loc_y());
                    if ((x%750)<250) d=180*(x>500)+375-30*(y<500);
               else if ((y%750)<250) d=180*(y>500)+105-30*(x<500);
               else if(damage()>75) d+=180;
               spara(drive(d,100));
               if(scan(a-15,10)) a-=4;
               if(scan(a+15,10)) a+=4;
               spara(drive(d,100)); if(r>342) fuoco(drive(d,100));
            }
         }
         else fire(a=ang,100);
      } 
   ...
   while(1)
   {
      if(r>40) d=a+80+(b^=1)*200; else d+=180;
      x=loc_x(y=loc_y());
           if ((x%850)<150) d=180*(x>500);
      else if ((y%850)<150) d=180*(y>500)+90;
      else if(damage()>75) d+=180;
      spara(drive(d,100));
      if(scan(a-15,10)) a-=4;
      if(scan(a+15,10)) a+=4;
...
int d,v;
   ...
   if (r=scan(oa=a,10))
   {
           if (scan(a-8,5)) {if (scan(a-=5,2)); else a-=4; return cannon(a+(a-oa),(scan(a,10)<<1)-r);}
      else if (scan(a+8,5)) {if (scan(a+=5,2)); else a+=4; return cannon(a+(a-oa),(scan(a,10)<<1)-r);}
      else
      {
         if (scan(a,2))
         {
            if (scan(a-=2,2)) return cannon(a+(a-oa),(scan(a,10)<<1)-r);
            else return cannon((a+=4)+(a-oa),(scan(a,10)<<1)-r);
         }
...
int asin,acos;
   ...
   if (scan(a,10))
   {
      ...
      else if (scan(a-20,10)) return firevs(a-=12,100);
      else if (scan(a+20,10)) return firevs(a+=12,100);
      else {while (!(scan(a+=20,10))); return firevs(a,100);}
   }
