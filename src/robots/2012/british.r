...
Scheda tecnica:
British è l'ennesima iterazione di neutron.r (del 2007),
con poche modifiche alle rotine di fuoco e al f2f.
...
main()
	...
	if(hor=(loc_x()>500))  dx(940); else sx(60);
...
fuoco(d)
{
	...
	if ((range=scan(oang=ang,10))) {
		if (scan(ang-8,5))  { if (scan(ang-=5,2)) ; else ang-=4; return(cannon(ang+(ang-oang),2*scan(ang,10)-range)); }
		if (scan(ang+8,5))  { if (scan(ang+=5,2)) ; else ang+=4; return(cannon(ang+(ang-oang),2*scan(ang,10)-range)); }
		if (scan(ang,10))     { if (scan(ang-=3,3)) ; else ang+=6; return(cannon(ang,2*scan(ang,10)-range)); }
