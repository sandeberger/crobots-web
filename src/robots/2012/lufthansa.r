...
Scheda tecnica:
Lufthansa è l'ennesima iterazione di proton.r (del 2007).
L'unica novità consiste nella scelta tra un oscillazione orizzontale
o verticale fatta all'inizio del match, con un controllo così
semplice che forse valeva la pena fare la scelta a caso.


*/


int range, dir, vdir, oang, ang;
int t, count;
...
main()
	...
	look();	
	if (scan(vdir,10)) 
	while(1) {		/*ciclo del 4vs4 orizzontale*/
		if (loc_x(++t)>500){
			while(speed()>49);
			drive(dir=180,100);
		...
		if((t%12)==count) look();	
		else if(dist>899) { look(); count = (t%12);}
	}
	...
	while(1) {	/*ciclo del 4vs4 verticale*/
			...
			if (loc_y(++t)>500){
				...
				while(speed()>49);
				drive(dir=90,100);
				if (!(dist=scan(100,10))) if (!(dist=scan(80,10))) dist=900+mod;
...
fuoco()
{
	...
	if ((range=scan(oang=ang,10))) {
		if (scan(ang-8,5))  { if (scan(ang-=5,2)) ; else ang-=4; return(cannon(ang+(ang-oang),2*scan(ang,10)-range)); }
		if (scan(ang+8,5))  { if (scan(ang+=5,2)) ; else ang+=4; return(cannon(ang+(ang-oang),2*scan(ang,10)-range)); }
		if (scan(ang,10))     { if (scan(ang-=3,2)) ; else ang+=6; return(cannon(ang,2*scan(ang,10)-range)); }
	} 
	else {
...
look() {
	...
	sca=sc+100;
	ne=2;
	while(sca>sc&&ne) ne-=(scan(sca-=20,10)>0);
