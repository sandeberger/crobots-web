...
Wall-E III eredita qualcosa da Wall-E II, qualche idea da Jedi10 (il movimento
triangolare) e l'F2F (diversamente parametrizzato) di Minion & C..
Rende meglio nel globale micro 90-2011 e peggio nei tornei annuali 2011
(chissene...).
*/
...
corner()
{
  if ((flag>>4)^posx^posy) runY(runX()); else runX(runY());
}

int look()
{
  if (scan(a=deg+90*(flag!=0),10)) ang=a;
  return 6;
}
...
main() /* Inizializza alcune variabili ed innesca la routine principale */
{
 ...
 dvector=(deg=(90*((posy<<1)+(posx^posy))))+30;
 ...
 while (1)
 {
        ...
        if (timer<2)
        {
         ...
         { 	/*attacco del f2f*/
          ...
          timer=look(dir=(dvector>>flag)&65535);
