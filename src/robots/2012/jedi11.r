...
Jedi11 è un Jedi10 con alcune modifiche alle routine di fuoco usate quando il
robot si avvicina ai bordi. Poi sono state modificate le costanti numeriche
usate nello spostamento, nel timing pre-F2F, e nell'F2F, usando un fottio di
bench test, cercando di guadagnare anche il minimo punto percentuale con i robot
del torneo90-2011. Siccome i robot futuri, ovviamente, non sono noti, queste
modifiche introdurranno sicuramente dei peggiornamenti....
*/
...
int
  ...
  dscan,dvector /* Array bidimensionali                             */
  ;
...
/* routine di fuoco del f2f di Proton */
fuoco()
{
...
{
  dir=180*!posx;
  if(posx) { while(loc_x()<910) fire3(drive(dir,100)); while(loc_x()<950) drive(dir,100); }
  else     { while(loc_x()> 90) fire3(drive(dir,100)); while(loc_x()> 50) drive(dir,100); }
  stop();
}
...
{
  dir=270-180*posy;
  if(posy) { while(loc_y()<910) fire3(drive(dir,100)); while(loc_y()<950) drive(dir,100); }
  else     { while(loc_y()> 90) fire3(drive(dir,100)); while(loc_y()> 50) drive(dir,100); }  
  stop();
}
...
main() /* Inizializza alcune variabili ed innesca la routine principale */
{
  while(1)
  {
    ...
    fastradar(deg2=(dvector=(dscan=deg1=(90*((posy<<1)+(posx^posy))))+30)+70);
    dvector|=((dvector+30)<<16);
    dscan|=((dscan+70)<<16);
    while (enemies<1) /* fase 1 */
    {
      dir=(dvector>>flag)&65535;
      if (look((dscan>>flag)&65535))
      {
        fire(dir);
		...
		while(1)
    {
      if ((posx=loc_x(posy=loc_y()))>880 ) dir=160+40*(posy>500);
			...
			else if (range>675) dir=ang+20+(b^=1)*210;
			else if (range<195) dir=ang+160+(b^=1)*15;
