...
Hal9012 è un mix della versione preliminare di Jedi11 ridotta, e una leggera
modifica di Hal9011. Non è stato, in pratica, veramente mai testato. Hal9012 è
  ...
  while(1)
  {
    ...
    while (enemies<1) /* fase 1 */
    {
      ...
      if ((++timer>tl)&&(range>400))
    	{
    	    /*if ((timer>110)&&(damage()<55)) enemies=1;
    ...
    while(enemies<=1) /* fase 2 */
    {
      ...
      if ((++timer%2)&&(range>400))
    	{
    	    /*if ((timer>128)&&(damage()<40)) enemies=2;
    	    else*/ fastradar();
      }
    }
    /* F2F di Proton */
