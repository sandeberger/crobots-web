/*
  Wizard v2 by Emanuele Marsigliani

  Wizard v2 è il bugfix della versione originale (Wizard)
  presente nel pacchetto dei robot "internazionali".
  Il fatto che la versione originale abbia un bug che lo fa
  schiantare contro l'angolo sud-est mi ha sempre dato
  fastidio, per cui ho voluto porre rimedio: Wizard2 è ...
  Wizard come sarebbe dovuto essere in origine.
*/
int x,count,change,dam;
int orange,ox,dir,range,rlead,laps,heading,posx,posy,xy,turn;
main()
{
  posy=loc_y(posx=loc_x()>500)>500;
  heading=posx<<1;
  dir=180*posx;
  drive(dir,100);  /*  Get Moving  */
 change=90;
