...
Come quasi tutti i crobot piu' recenti Rudolf_6 comincia l'incontro
raggiungendo l'angolo piu'vicino (spostandosi prima verticalmente e poi
orizzontalmente). Qui la strategia e' abbastanza varia:

-Se i danni sono superiori all'80% il movimento e' molto simile a quello
 dei primi tre Rudolf, con oscillazioni strette diagonali e routine
 di sparo grossolana.

-Altrimenti l'oscillazione e' parallela ad un lato e la scelta della direzione
...
if (dor<dver){
     ...
     while(speed()>49);
     drive(180,100);
     if (!oldr||oldr>700) {if(!aa) {deg=180;aa=1;}} else if (oldr<800) adef();
     ...
     while(speed()>49);
     drive(0,100);
     if (!oldr||oldr>700) {if(!aa) {deg=0;aa=1;}} else if (oldr<800) bdef();
     ...
     while(speed()>49);
     drive(270,100);
     if (!oldr||oldr>700) {if(aa) {deg=270;aa=0;}} else if (oldr<800) cdef();
     ...
     while(speed()>49);
     drive(90,100);
     if (!oldr||oldr>700) {if(aa) {deg=90;aa=0;}} else if (oldr<800) ddef();
...
piccolo()
{
if (b==0) {vai(100,100);vai(50,50);ang=45;ly=80;}
else if (b==1) {vai(900,100);vai(950,50);ang=135;ly=80;}
else if (b==2) {vai(900,900);vai(950,950);ang=45;ly=920;}
else {vai(100,900);vai(50,950);ang=135;ly=920;}
while(t<10000){
++t;
                        while (loc_y()<=ly) {drive(ang,100);Vuoco(); if (oldr>900) deg+=40;}
                        ang+=180;
                        drive(ang,0);
                        while (loc_y()>ly) {drive(ang,100);Vuoco(); if (oldr>900) deg+=40;} 
                        ang-=180;
...
vai(x,y)
{
    ...
    while(speed()>49) Fire();
    dir=deg(x,y);
    while (dist(x,y)>900) {drive (dir,100);Vuoco();}
    drive(dir,0);
}
