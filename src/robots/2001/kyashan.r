...
Kyashan e' un microcrobots molto semplice: all'inizio dell'incontro si posiziona 
nell'angolo in basso piu' vicino e si muove lungo i lati di un triangolo.
...
int r,t,last,ang,dis,contacicli,angpos;
...
main()
{
	...
	/* si sposta nell'angolo in basso più vicino */
	if(loc_x()>500)	
	{	
		angpos=0;
	...
	while(last>1)
	{
		...
		while(--t)
		{
			...
			{
				dis=250-damage();
				while(loc_x()<dis) fuoco(0);
				while(loc_x()>75) fuoco(135);
				while(loc_y()>100) fuoco(270);
			}
		}
	...
	if(angpos==0) while(loc_x()>500) fuoco(180);
	else  while(loc_x()<500) fuoco(0);
	/* attacco finale */
     	while(1)
     	{
