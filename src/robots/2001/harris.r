...
Dopo 1180 chiamate alle funzioni di sparo attacca i superstiti
    ...
    while (danni=damage()+25) {
            ...
            while (damage()<danni) {
                ...
                if (loc_x()<500) if (loc_y()<500)
                         { xsmin(180,0); xsmag(100,135); ysmag(100,270); }
                         else
                         { ysmag(820,270); ysmin(900,45); xsmag(100,180); }
                else if (loc_y()<500)
                         { ysmin(180,90); ysmag(100,225); xsmin(900,0); }
                         else
                         { xsmag(820,180); xsmin(900,315);ysmin(900,90); }
                }
                drive (dir,0);
                if(!orng||orng>850) {
                        i=-10; ne=0;
...
xsmag(x,d) { dir=d; while(loc_x()>x) spara(drive(d,100)); drive(d,0); }
xsmin(x,d) { dir=d; while(loc_x()<x) spara(drive(d,100)); drive(d,0); }
ysmag(y,d) { dir=d; while(loc_y()>y) spara(drive(d,100)); drive(d,0); }
ysmin(y,d) { dir=d; while(loc_y()<y) spara(drive(d,100)); drive(d,0); }

xfmag(x,d) { dir=d; while(loc_x()>x) fuoco(drive(d,100)); drive(d,0); }
xfmin(x,d) { dir=d; while(loc_x()<x) fuoco(drive(d,100)); drive(d,0); }
yfmag(y,d) { dir=d; while(loc_y()>y) fuoco(drive(d,100)); drive(d,0); }
yfmin(y,d) { dir=d; while(loc_y()<y) fuoco(drive(d,100)); drive(d,0); }
