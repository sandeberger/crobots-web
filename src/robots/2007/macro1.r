...
Identico a !alien con attacco finale diverso (quadrato grande!) e lievi ritocchi nei parametri.
...
LookZ()
{ 
  if (timer>180) { if (!Look(zd)) if (!Look(zd)) timer=10000; if (damage()<30) timer=10000; }  else if (damage()>25) timer=10000;   
}
...
Stop(d)
{
  ...
  if (scan(deg+10,10)) deg+=5; else deg+=355; 
  if (scan(deg+350,10)) deg+=357; else deg+=3; 
  if (scan(deg+10,10)) deg+=2; else deg+=358; 
