...
*/
    ...
    while (dam=damage()+10) {
        while ( (damage() < dam) ) { /*fino quando non e' colpito o limite di tempo*/
                ...
                if(orng>740) if (ne==0||t>1150) attacco();
...
{
        if(dx) {
                if(occupato(170)) seek(att=101);
                else step(0);
        } else {
                if(occupato(350)) seek(att=101);
                else step(0);
        }
}
...
int occupato(w)
/* restituisce 1 se ci sono nemici nella direzione w */
int w;
{
...
xyf()  { sk=1;spara(drive(dir,100)); if(t>1150&&damage()<40) fuoco(drive(dir,100));spara(drive(dir,100));sk=0;stop(dir);} 
