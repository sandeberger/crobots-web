...
{
	if (!((loc_x() > 600) && (loc_y() < 600))) {
		dir = plot_course (950, 200);
		ang = dir;
		while ((loc_x() < 900) && (loc_y() > 250)) /* vai a casa... */
			shoot (100);                         /*...sparando */
	...
	while (1) {
		...
		dir = plot_course (50, 300);  /* prossima destinazione */
		if (dist == 0)               /* Nessuno in vista? */
			ang = dir;             /* Avanti! */
		while (loc_x() > 170)        /* Controlla l'arrivo */
		...
		dir = plot_course (650, 950);  /* prossima destinazione */
		if (dist == 0)               /* Nessuno in vista? */
			ang = dir;             /* Avanti! */
		while (loc_y() < 890)        /* Controlla l'arrivo */
			shoot (100);           /* sparando al massimo */
		drive (dir, 40);              /* Sei vicino, rallenta! */
		if (dist == 0)               /* Se non ci sono nemici in vista */
			ang = dir;             /* Aggiusta la direzione */
		...
		dir = plot_course (950, 200); /* prossima destinazione */
		if (dist == 0)               /* Nessuno in vista? */
			ang = dir;             /* Avanti! */
		while (loc_y() > 270)        /* Controlla l'arrivo */
			shoot (100);           /* sparando al massimo */
		drive (dir, 40);              /* Sei vicino, rallenta! */
		if (dist == 0)               /* Se non ci sono nemici in vista */
			ang = dir;             /* Aggiusta la direzione */
