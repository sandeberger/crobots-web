...
/*            inquanto, essendo già bassa la precisione del  */
...
main()
{
	...
	/* se il Crobot fosse già nelle immediate vicinanze, vi  */
	/* viene lasciato                                        */
	if (loc_x() < 200) { 
		drive(0, 100);
		while (loc_x() < 250) 
	...
	/* nel ciclo principale il crobot percorrerà il quadrato    */
	/* di coordinate (300,300)-(300,700)-(700,700)-(700,300)    */
	/* ripetutamente. Ciò lo porta sempre vicino a quei Crobots */
	...
	while (1) {
		...
		drive(180, 0);
		while (speed() > 49) 
			spara();     /* è stato raggiunto l'angolo */
		/* in alto a sinistra         */
		drive(0, 100);
		while (loc_x() < 660) 
			spara();
