...
/* Autore:      Turino Andrea
   ...
   SCHEDA TECNICA DEL ROBOT(ROBOT1):

   Il robot si porta sul lato basso dello schermo e si continua a
   muovere, rendendo pi— arduo cos il 'lavoro' degli altri C robots.
   Vi sono due routine di fuoco, una per quando il robot si sta fermando
   (spf) e l'altra per quando il robot si muove (spm). La prima cerca di
...
main()
{
    ...
    {
          /* se il time Š maggiore del limite guarda quanti robot sono
             rimasti*/
          if (time>8)
             if((time>20 && damage()<25) || nrobot()<=1)
                circle();
          /* cambia angolo */
          move();
          time+=1;
