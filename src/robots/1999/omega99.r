/*
  ...
  ======  Crobot OMEGA99  per scontri a 4 - by L.Cimini 26-11-1999  ======
...
TECHNICAL CARD:
  ...
  The routine of fire, already note, it has stayed modified for increase his
  efficiency, the routine is able to behavior 4 types of draught, from firm,
  in movement and for near target or far, the scanning could be rapid or
  normal, the type of draught has selected in base to the distance, but also
  using like selector the function speed() and a flag for normal express. 
...
SCHEDA TECNICA:
  ...
  La routine di fuoco, gia` nota, è stata modificata per aumentare la sua
  efficienza, la routine è in grado di fare 4 tipi di tiro, da fermo,
  in movimento e per bersaglio vicino o lontano, la scansione può essere
  rapida o normale, il tipo di tiro è selezionato in base alla distanza,
  ma anche usando come selettore la funzione speed() ed un flag per rapido
  normale.
...
main()
{
   ...
   while(1)
   {
      ...
      while(--t)
      {
         fuoco(0); if (damage()>(danno+3)) { scappa(); danno=damage();}
      }
      tt-=100;
      ...
      if (n<3)
      {
         if (damage()<88) finale(); else oscilla();
      }
      else if (tt<1)
      {
         if ((n<5) && (damage()<60))
         {
            oscilla(); if (damage()<30) finale();
         }
      }
   }
}
...
sx(x) {d=180; while(loc_x()>x) {drive(d,100); fuoco(1);} drive(d,0); }
dx(x) {d=0;   while(loc_x()<x) {drive(d,100); fuoco(1);} drive(d,0); }
dn(y) {d=270; while(loc_y()>y) {drive(d,100); fuoco(1);} drive(d,0); }
up(y) {d=90;  while(loc_y()<y) {drive(d,100); fuoco(1);} drive(d,0); }
...
oscilla()
{
   ...
   {
      if (loc_y()< 500) { up(266); dn(100); } else { dn(733); up(900); }
   }
}
...
finale()
{
   ...
   while(1)
   {
      while (loc_x()<866) { drive (d,100); fuoco(0); } d+=180;
      while (loc_x()>133) { drive (d,100); fuoco(0); } d+=180;
   }
}
...
fuoco(f)
{
   ...
   else if (scan(a+339,10)) a+=339;
   else if (scan(a+21, 10)) a+=21;
   else if (f) {a+=42; return;}
   else if (scan(a+318,10)) a+=318;
   else if (scan(a+42, 10)) a+=42;
   else a+=105;
}
